...
### ABOUT: ################
#
# Billy's bash script to optimize an array of zmatrices. because he's so dang lazy.
#
...
#		a. used for automating the repeated optimization of the zmatrix to convergence.
...
declare START="9.90"  # the initial bong length
declare FINISH="10.10" # the final bond length
...
declare ATOMNUMBER="$( echo $(( $LINENUMBER - 1 )) )"
...
createNewDir()
{
	...
	X_STRING8="$( printf "%-8s" X.XX )" # guarantees X.XX to be 8 chars in length
	BOND_LENGTH8="$( printf "%-8s" ${BOND_LENGTH} )" # guarantee the $BOND_LENGTH to be 8 chars in length

	sed -i "${LINENUMBER}s/${X_STRING8}/${BOND_LENGTH8}/" ${NAME}.z
	...
	#sed -i "${LINENUMBER}s/${BOND_LENGTH}/X.XX/" ${TEMPORARY_ZMAT}
}
